...
//$query="SELECT * FROM deposit WHERE branch_id='$branch_id'";
...
<html lang="en">
 ...
 <div class="container" style="border:1px solid #39d;">
			...
			<ul class="nav nav-pills nav-justified">
				<li class="active">
                    <a class="btn btn-success btn-large " href="#totaldeposit" style="margin-right:30px" data-toggle="tab">Deposit</a></li>
                 <li>   <a class="btn btn-primary btn-large" href="#totalpayment" style="margin-right:30px" data-toggle="tab">Payment</a></li>
            ...
            <div class="tab-content clearfix">
                <div class="tab-pane fade in active" id="totaldeposit" style="margin-top:10px;">
					...
					<div class="form-group">
						...
						<img id="imgSearch"  src="images/cancel.png" alt="Cancel Search" title="Cancel Search" style="width:150px;width:25px;height:20px;" />
					</div>
					
					
						<!--<div class="form-group">
						<label for="mname">Member Name</label>
						<input type="text" class="form-control" name="mname" id="mname" placeholder="Search By Member Name" onchange="callajax()">
						</div>-->
					...
					<tr>
                        ...
                        <td><?php echo $row1['slip_no'];?></td>
                        <td><?php echo $row1['date'];?></td>
						<td><?php echo $row1['source_branch_name'];?></td>
						<?php if($row1['branch_id']!=$row1['source_branch_id']){
						...
						<?php if($row1['deposite_type']=='Approved'){?>
										...
										<?php }else if ($row1['deposite_type']=='Rejected'){?><td style="color:red;font-weight:bold;"><?php echo $row1['deposite_type'];?></td><?php }
										else{?>
        ...
        <div class="tab-pane fade" id="totalpayment" style="margin-top:10px;">
				<div id="exampleForm1"> 
					...
					<div class="form-group">
						...
						<div class="form-group pull-right" style="margin-right:10px;">
							<label><input type="radio" name="payment" id="payment" value="Reject"  onchange="callajax1()">Rejected</label>
						</div>
						
						<div class="form-group pull-right" style="margin-right:10px;">
							<label><input type="radio" name="payment" id="payment" value="Approve" onchange="callajax1()">Approved</label>
						</div>
  				
   				
						<div class="form-group pull-right" style="margin-right:10px;">
							<label><input type="radio" name="payment" id="payment" value="all" checked onchange="callajax1()">ALL</label>
						</div>
					</form>
					</div><!-- End of Example Form-->
            ...
            <span class="allpayment">
					...
					<table id="tblPayment" class="table table-striped table-bodered responsive-table">
                    <thead>
					<tr>
						...
						if($row1['source_branch']!=$row1['destination_branch']){
							$from_branch = $row1['source_branch'];
							$branch_name = "SELECT branch_name from branch where branch_id = '$from_branch'"; 
							$result = $con->query($branch_name);
							$Frombranch = $result->fetch_all(MYSQLI_ASSOC);
							echo "<td>". $Frombranch[0]['branch_name']. "</td>";
						}else {
											echo "<td>Self</td>";
										}
						...
						<?php if($row1['branch_id']!=$row1['source_branch']){
							$to_branch = $row1['branch_id'];
							$branch_name = "SELECT branch_name from branch where branch_id = '$to_branch'"; 
							$result = $con->query($branch_name);
							$branchName = $result->fetch_all(MYSQLI_ASSOC);
