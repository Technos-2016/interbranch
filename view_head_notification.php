...
if ($branchnotificationcount > 0) {
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $response = $response ."<tr class='info' style='font-weight:bold;font-size:12px;'>"."<td class='notification-subject' width=10%>" . $row["displaydate"] . "</td>" . "<td class='notification-subject'>" . $row["branch_code"] . "</td>"; 
		
		$response = $response ."<td class='notification-subject'>" . $row["branch_name"] . "</td>" . "<td class='notification-subject'>" . $row["member_code"] . "</td>" . "<td class='notification-subject'>" . $row["amount"] . "</td>" . "<td class='notification-subject'>" . $row["member_name"] . "</td>" . "<td class='notification-subject'>" . $row["deposite_type"] . "</td>"."<td class='View Detail'><a class='btn btn-sm btn-primary' href='view_headquarter_detail.php?transactionId=".$row['transaction_id']."&memberId=".$row['member_id']."&branchId=".$row['branch_id']."'>View Detail</a></td>" . "</tr>";
         }
         else{
             $response = $response ."<tr >"."<td class='notification-subject' width=10%>" . $row["displaydate"] . "</td>" . "<td class='notification-subject' width=10%>" . $row["branch_code"] . "</td>"; 
